...
goog.provide("Blockly.Blocks.sql");
...
Blockly.Blocks["query"] = {
  init: function() {
    this.appendValueInput("VALUE1")
      .setCheck("option")
      .appendField(new Blockly.FieldLabel("query"), "option");
    this.setColour(160);
  }
};
...
Blockly.Blocks["where"] = {
  ...
  var OPERATORSComulms = [
      ["", ""],
    ];
    let dorpDown = new Blockly.FieldDropdown(OPERATORS_RANGE);
    let dorpDownOpt = new Blockly.FieldDropdown(OPERATORS);
    let dorpDownComulms = new Blockly.FieldDropdown(OPERATORSComulms);
    var me=this;
    //重写点击方法
    dorpDownComulms.showEditor_ = function() {
     dorpDownComulms.menuGenerator_=Blockly.tableColums[me.parentBlock_.parentBlock_.getFieldValue('tableName')]
      ...
      goog.events.listen(b, goog.ui.Component.EventType.ACTION, function(b) {
        if ((b = b.target))
          (b = b.getValue()), a.sourceBlock_ &&
            (b = a.callValidator(b)), null !== b && a.setValue(b);
        Blockly.WidgetDiv.hideIfOwner(a);
      });
      b
        .getHandler()
        .listen(b.getElement(), goog.events.EventType.TOUCHSTART, function(a) {
          this.getOwnerControl(a.target).handleMouseDown(a);
        });
      b
        .getHandler()
        .listen(b.getElement(), goog.events.EventType.TOUCHEND, function(a) {
    ...
    this.appendValueInput("VALUE1")
      .setCheck("option")
      .appendField(
        new Blockly.FieldLabel("where"),
        function() {
          console.log(1);
        },
        "option"
      )
      .appendField(dorpDown, "range")
      .appendField(dorpDownComulms, "comulms")
      .appendField(dorpDownOpt, "opt")
...
Blockly.Blocks["where_item"] = {
  init: function() {
    this.appendValueInput("VALUE1")
      .setCheck("option")
      .appendField(new Blockly.FieldLabel("where"), "option");
    this.setColour(160);
    this.setPreviousStatement(true);
    this.setNextStatement(true);
    this.contextMenu = false;
...
Blockly.Blocks["where_join"] = {
  /**
   * Block for creating a string made up of any number of elements of any type.
   * @this Blockly.Block
   */
  init: function() {
    this.setHelpUrl(Blockly.Msg.WHERE_JOIN);
    this.setColour(Blockly.Blocks.texts.HUE);
    this.itemCount_ = 1;
    this.updateShape_();
    this.setOutput(true, "condition");
    this.setMutator(new Blockly.Mutator(["where_item"]));
    this.setTooltip(Blockly.Msg.WHERE_JOIN);
  },
  /**
  ...
  decompose: function(workspace) {
    var containerBlock = workspace.newBlock("where_container");
    containerBlock.initSvg();
    var connection = containerBlock.getInput("STACK").connection;
    for (var i = 0; i < this.itemCount_; i++) {
      var itemBlock = workspace.newBlock("where_item");
      itemBlock.initSvg();
  ...
  updateShape_: function() {
    ...
    for (var i = 0; i < this.itemCount_; i++) {
      if (!this.getInput("ADD" + i)) {
        ...
        if (i == 0) {
          input.appendField(Blockly.Msg.WHERE_JOIN);
        }
      }
    }
...
Blockly.Blocks["text_indexOf"] = {
  ...
  init: function() {
    ...
    var tooltip = Blockly.Msg.TEXT_INDEXOF_TOOLTIP.replace(
      "%1",
